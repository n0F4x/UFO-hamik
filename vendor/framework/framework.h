...
inline mat4 RotationMatrix(float angle, vec3 w) {
    ...
    w = normalize(w);
    return mat4(vec4(c * (1 - w.x * w.x) + w.x * w.x,
                     w.x * w.y * (1 - c) + w.z * s,
                     w.x * w.z * (1 - c) - w.y * s,
                     0),
                vec4(w.x * w.y * (1 - c) - w.z * s,
                     c * (1 - w.y * w.y) + w.y * w.y,
                     w.y * w.z * (1 - c) + w.x * s,
                     0),
                vec4(w.x * w.z * (1 - c) + w.y * s,
                     w.y * w.z * (1 - c) - w.x * s,
                     c * (1 - w.z * w.z) + w.z * w.z,
                     0),
                vec4(0, 0, 0, 1));
...
class Texture {
    //---------------------------
    std::vector<vec4>
    load(std::string pathname, bool transparent, int& width, int& height) {
        ...
        unsigned int size = (unsigned long)bitmapFileHeader[17]
                          + (unsigned long)bitmapFileHeader[18] * 65536;
        ...
        {   // Swap R and B since in BMP, the order is BGR
            float alpha = (transparent)
                            ? (bImage[idx] + bImage[idx + 1] + bImage[idx + 2])
                                  / 3.0f / 256.0f
                            : 1.0f;
            image[i++] = vec4(bImage[idx + 2] / 256.0f,
                              bImage[idx + 1] / 256.0f,
                              bImage[idx] / 256.0f,
                              alpha);
        }
...
public:
    ...
    void create(int width,
        ...
        glTexImage2D(GL_TEXTURE_2D,
                     ...
                     &image[0]);                               // To GPU
        glTexParameteri(
            GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, sampling);   // sampling
...
class GPUProgram {
    ...
    bool checkShader(unsigned int shader, std::string message) {   // check if
                                                                   // shader
                                                                   // could be
                                                                   // compiled
        int OK;
    ...
    bool checkLinking(unsigned int program) {   // check if shader could be
                                                // linked
        int OK;
    ...
    int getLocation(const std::string& name) {   // get the address of a GPU
                                                 // uniform variable
...
public:
    ...
    bool create(const char* const vertexShaderSource,
        ...
        glShaderSource(
            vertexShader, 1, (const GLchar**)&vertexShaderSource, NULL);
        ...
        if (geometryShaderSource != nullptr) {
            ...
            glShaderSource(
                geometryShader, 1, (const GLchar**)&geometryShaderSource, NULL);
        ...
        glShaderSource(
            fragmentShader, 1, (const GLchar**)&fragmentShaderSource, NULL);
        ...
        glBindFragDataLocation(
            shaderProgramId, 0, fragmentShaderOutputName);   // this output goes
                                                             // to the frame
                                                             // buffer memory
